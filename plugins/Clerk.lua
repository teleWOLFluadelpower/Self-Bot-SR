...
if msg then
    ...
    if  not redis:get(bot.id..'Time:'..msg.sender_user_id)  and is_private(msg) and Get('other','ClerkMod') and Get('other','clerkTYPE') and Get('other','Self-OFFED')  then
        if Get('other','clerkTYPE') == 'Message' then
  return tdbot.sendText(msg.chat_id, msg.id, Get('other','clerkID'), 'md', false, false, false, 0, nil, nil, nil)
elseif Get('other','clerkTYPE') == 'Gif' then
     id = Get('other','clerkID')
          return  tdbot.sendAnimation(msg.chat_id, msg.id, id, '*Sorry :(*', 'md', nil, 0, 0, nil, 0, 0, false, trye, nil, nil, nil)
    elseif Get('other','clerkTYPE') == 'VoiceNote' then
        id = Get('other','clerkID')
      return   tdbot.sendVoiceNote(msg.chat_id, msg.id, id,  '*Sorry :(*', 'md', 4, 'test', true, 0, nil, nil, nil)
        elseif Get('other','clerkTYPE') == 'VideoNote' then
            id = Get('other','clerkID')
           return tdbot.sendVideoNote(msg.chat_id, msg.id, id, 0, 0, 0, 0, 0, true, true, nil, nil, nil)
        end
...
return {patterns = patterns_,runing = ClerkMod,run = PreMessage}